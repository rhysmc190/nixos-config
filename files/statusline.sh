...
STATUSLINE_VERSION="1.3.2-improved"

# ---- Configuration ----
# Claude Code adds extra info (thinking status, active file) to the right side of line 1
# Reserve space to prevent wrapping when extra info appears
...
# Keep this line short to leave room for Claude Code's extra info (thinking, active file)
...
# Line 3: Session usage analytics
line3=""

# Session usage percentage with progress bar (estimated from ccusage projection)
if [ -n "$session_usage_pct" ]; then
  ...
  line3="📊 $(usage_bar_color)Session Usage Est: ${session_usage_pct}% [${session_usage_bar}]$(rst)"
fi
